...
(function(root, mod) {
  ...
  //A constructor function for contexts. defs should be an array of type definition objects, 
  ...
  exports.Context = function(defs, parent) {
    ...
    //Calls f with the current context bound to context. Basically, 
    //all code that does something with the inference engine should be wrapped in such a call. 
    exports.withContext(this, function() {
    	//Constructor for the type that represents JavaScript objects. 
    	//proto may be another object, or true as a short-hand for Object.prototype, 
    	//or null for prototype-less objects. 
      cx.protos.Object = new Obj(null, "Object.prototype");
      cx.topScope = new Scope();
      cx.topScope.name = "<top>";
      cx.protos.Array = new Obj(true, "Array.prototype");
      
      //Constructor for the type that implements functions. Inherits from Obj. 
      //The AVal types are used to track the input and output types of the function. 
  ...
  Scope.prototype = extend(Obj.prototype, {
    ...
    //(defining it in the top scope if it is missing), 
    //and, if the property doesn’t already have an originNode, assigns the given node to it. 
  ...
  //Parse a piece of code for use by Tern. Will automatically fall back to the error-tolerant 
  //parser if the regular parser can’t parse the code. 
  ...
  //Analyze a syntax tree. name will be used to set the origin of types, properties, 
  //and variables produced by this code. The optional scope argument can be used to specify 
  //a scope in which the code should be analyzed. It will default to the top-level scope. 
  ...
  //if start is given, starts at the given start offset. 
  //scope can be given to override the outer scope, which defaults to the context’s top scope. 
  //Will return a {node, state} object if successful, where node is AST node, 
  //and state is the scope at that point. Returns null if unsuccessful. 
  ...
  //Similar to findExpressionAround, except that it will return the innermost expression
  ...
  //Determine an expression for the given node and scope (as returned by the functions above). 
  //Will return an AVal or plain Type.
  ...
  //Will traverse the given syntax tree, using scope as the starting scope, looking for references 
  //to variable name that resolve to scope refScope, and call f
  ...
  //Analogous to findRefs, but used to look for references to a specific property instead. 
  //Whereas findRefs is precise, 
  //this is dependent on type inference, and thus can not be relied on to be precise. 
  ...
  //Find the scope at a given position in the syntax tree. The scope parameter can be used to override 
  //the scope used for code that isn’t wrapped in any function. 
